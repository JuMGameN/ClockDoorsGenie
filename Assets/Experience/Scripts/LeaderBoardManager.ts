...
export default class LeaderboardManager extends MonoBehaviour {
    ...
    private Start() {
        GameManager.Instance.OnWinGame.addListener(() => this.saveScoreToLeaderboard());
    }

    private async saveScoreToLeaderboard() {
        console.log("saveScoreToLeaderboard");
        let currentScore = ScoreManager.Instance.getScore();
        GeniesLeaderboardSdk.SubmitCurrentUserScoreAsync(this.leaderboardId, currentScore, TypeEnum.DIRECT);
